...
| **R13** | **Sum and floor.** `total = phys + fire + cold + lightning + magic` (poison is *not* summed here — it is seeded as a DoT in R14). If any component was greater than 0 before mitigation and `total < 1`, set `total = 1`. Then `total = ⌊total⌋`. |
