...
export const SHOTS = {
  ...
  dense_combat: {
    id: 'dense_combat',
    description: "Ashen Wastes, runIndex 18 (world seed 0, `world.seedFor('ashen_wastes',18)` = 0xb9a10dca) with AI-7's real zone:ready spawn pass: the player standing at the centre of the zone's largest pack while it and its neighbours converge. Measured headlessly on exactly this sequence: 9 packs / 68 monsters placed by world's WRLD-9 spawn plan, 6 packs / 47 monsters inside 06 §10.3's 34 m activation radius, 24 monsters within 25 m of the player, 17 within 18 m, 16 within 12 m, the 8-strong anchor pack (carrion_swarm) closed from 2.38-3.98 m to 0.03-0.65 m over the 200 pumped steps (each member really walked 1.93-3.94 m; measured at exactly step 200, not extrapolated). INCOMPLETE against 12-testing.md §9.1's own definition of this shot, deliberately and by scope: §9.1 asks for '22 monsters, 4 skill effects, 9 ground items'. The monster half is here (24 within 25 m). The other two thirds are NOT: no `fx` subsystem exists in this codebase at all (so no skill effect can be drawn), and nothing in this frame drops loot (no kill happens — see below), so there are 0 ground items. §9.1 also calls this 'the readability gate AND the performance gate' for readability.mjs/profile.mjs; as registered here it can serve neither yet. Whoever owns FX-5 and the loot half must extend THIS setup rather than register a second shot — 12 §9.3's own sanctioned 'a shot's setup grows, the name doesn't' pattern, the same way inventory_full grew in M4. READ THIS BEFORE READING THE PIXELS — the blocker AI-7 reported here is CLOSED, and a second, larger one took its place. (1) O-112 is closed: `registry.add(AiSystem)` landed in src/main.js, and the simulation behind this frame is real, measured on the live page through boot()'s own registry — 9 packs, 52 monsters, spawnPushed 0, ranks {normal 45, champion 2, unique 1, minion 4}, so AI-8's promotion and affixes reach the running application and not merely the tests. (2) O-128: NONE of those 52 monsters is drawn, and none can be. `actor.view.root` is assigned in exactly one place in the whole tree — src/actors/pool.js:568, `actor.view.root = null` — and nothing builds an actor view or attaches one to ctx.scene. Asked of the live page: 52 monsters, 52 with a `view` record, 0 with a `root`, 0 attached to the scene; the scene holds 23 meshes and every one of them is a prop (boulder_small, ruin_arch, column_drum, ...). The player has no view either. The `actor_ranker` shot builds its own mesh inside its own setup and adds it to the scene by hand, which is why M2 could bless a frame with a monster in it while the running game shows none. So: every black silhouette in this frame is a PROP, the pale capsule is src/dev/debugview.js's placeholder, and the monsters are present in simulation and absent in pixels. This shot therefore cannot serve as 12 §9.1's readability gate until the ACTR-16..ACTR-19 / MATL-3 lane (backlogged AFTER M5) attaches actor views. Blessed with that stated, not with it hidden. Same rendering caveats as wastes_seed_a/b, verbatim: no `sky` subsystem exists, so no light reaches any MeshStandardMaterial and every monster, prop and ridge renders as a flat black silhouette; the green ground plane and the pale capsule are src/dev/debugview.js's unlit MeshBasicMaterial placeholders, not this ticket's geometry. Every pack in frame is a SINGLE archetype rather than a mixed roster — src/world/spawn.js draws PackDescriptor.archetypeId uniformly from descriptor.bestiary (bestiary ids) where 06 §5.1 specifies S3.weighted(bestiaryWeights(cell)) (template ids), so ai.spawnPack correctly takes §5.1's 'a bestiary id spawns count identical monsters' branch and no mixed pack can appear in any frame yet; a world-side gap AI-7 reported and did not fix. No damage is exchanged in this frame: the pack is converged and swinging, nothing drives a player attack.",
    milestone: 'M5',
    steps: 200,
    // Zero free variables — see this file's header, "pumpShot — AND a
